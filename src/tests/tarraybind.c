...
TOLUA_API int tolua_tarray_open(lua_State* tolua_S)
{
	...
	int top1 = lua_gettop(tolua_S);
	tolua_module(tolua_S, NULL, 1);   //生成一个module(表) 如果有变量 _G add metatable,and,metatable add __index && __newindex
	int top2 = lua_gettop(tolua_S);
	tolua_beginmodule(tolua_S, NULL);  // 将module表放到栈顶
	int top3 = lua_gettop(tolua_S);
#ifdef __cplusplus
	tolua_cclass(tolua_S, "Point", "Point", "", tolua_collect_Point);
#else
	tolua_cclass(tolua_S, "Point", "Point", "", NULL);
	int top4 = lua_gettop(tolua_S);
#endif
	tolua_beginmodule(tolua_S, "Point");
	int top5 = lua_gettop(tolua_S);
	tolua_variable(tolua_S, "x", tolua_get_Point_x, tolua_set_Point_x);
	tolua_variable(tolua_S, "y", tolua_get_Point_y, tolua_set_Point_y);
	int top6 = lua_gettop(tolua_S);
